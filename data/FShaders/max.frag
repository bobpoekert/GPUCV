...
void main(void)
{
//note: use only one channel image
    float img1 = GETTEX(BaseImage, gl_TexCoord[0].st).r;
    float img2 = GETTEX(Image0, gl_TexCoord[0].st).r;
    gl_FragColor = vec4(max(img1,img2),0.,0., 1.);  
}
