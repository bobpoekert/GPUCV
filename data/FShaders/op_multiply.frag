...
uniform float Parameters[1];


void main(void)
{
    vec3 img1 = GETTEX(BaseImage, gl_TexCoord[0].st).rgb;
    vec3 img2 = GETTEX(Image0, gl_TexCoord[0].st).rgb;
    float Factor= Parameters[0]*256.;
	gl_FragColor = vec4(Factor*(img1.r * img2.r),Factor*(img1.g * img2.g),Factor*(img1.b * img2.b	),1.);  
}
