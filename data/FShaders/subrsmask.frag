...
void main(void)
{
    vec3 img = GETTEX(BaseImage, gl_TexCoord[0].st).rgb;
   if (GETTEX(Image0, gl_TexCoord[0].st).r != 0.)
   gl_FragColor = vec4 ( Parameters[0] - img.r , Parameters[1] - img.g   , Parameters[2] - img.b , 1.);
   else gl_FragColor = GETTEX(Image0, gl_TexCoord[0].st);
}
