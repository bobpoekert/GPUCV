...
void main(void)
{
	...
	vec2 delta={Parameters[0]/Parameters[2]/2., Parameters[1]/Parameters[3]/2.};
	vec2 PPdelta = {1./Parameters[0], 1./Parameters[1]};
	
	vec2 PP = gl_TexCoord[0].st;
//	vec4 colorx = GETTEX(BaseImage, gl_TexCoord[0].st);
//	vec4 colory = GETTEX(BaseImage, gl_TexCoord[0].st);
//	vec4 colorxTemp,coloryTemp ;
	vec4 colorTemp;
   ...
   for (float x=-(PPdelta.x*delta.x); x < PPdelta.x*delta.x; x+=PPdelta.x)
   {
    for (float y=-(PPdelta.y*delta.y); y < PPdelta.y*delta.y; y+=PPdelta.y)
    {
     PP = gl_TexCoord[0].st+vec2(x,y);
     //if ((PP.x>=0) && (PP.x <=1))
     // if ((PP.y>=0) && (PP.y <=1))
	 // {
		colorMask = GETTEX(Image0,PP);
		if (colorMask.rgb!= vec3(0.))
   		{
