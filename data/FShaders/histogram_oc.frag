...
void main(void)
{
	float step = (Parameters[1] - Parameters[0])/Parameters[2];
	float actual_position = Parameters[3]*step+Parameters[0];
	vec4 current_color = GETTEX(BaseImage, gl_TexCoord[0].st);
		
	float lum = current_color.b;

	if((lum >= actual_position)
	&&(lum < (actual_position+step)))
